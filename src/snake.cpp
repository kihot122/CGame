...
c_Snake::~c_Snake()
{
    ...
    while (x!=tail.end())
    {
        playground->SetCell(x[0].X,x[0].Y,new c_Cell);
        *x++; 
    }
...
void c_Snake::Move(){
    ...
    switch ((playground->GetCell(next_Cell.X,next_Cell.Y))->GetType())
    {
    case CELL_EMPTY:
        MoveHead(next_Cell);
        MoveBack();
...
{
    playground->SetCell(tail.back().X,tail.back().Y,new c_Cell);
    tail.pop_back();
}
